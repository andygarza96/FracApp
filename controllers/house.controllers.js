...
exports.getHouseById = asyncHandler(async (req, res, next) => {
    ...
    if (houseModel) {
        ...
        return next(
            new ErrorResponse(`House not found with id of ${req.params.houseId}`, 404)
        );
    }
});
...
exports.updateHouse = asyncHandler(async (req, res, next) => {
    ...
    const updatedHouse = await HouseModel.findByIdAndUpdate(req.params.houseId, {
...
exports.deleteHouse = asyncHandler(async (req, res, next) => {
    const deletedHouse = await HouseModel.findByIdAndDelete(req.params.houseId);
